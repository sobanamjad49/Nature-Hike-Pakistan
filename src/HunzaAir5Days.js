...
import { AirHunza5DaysImages, AirHunza5DaysData } from "./travelData";

function HunzaAir5Days() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "HunzaAir5Days", // FIXED TOUR NAME
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {AirHunza5DaysImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Hunza by air 5 days"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            5 DAYS BY-AIR HUNZA, BORITH, ATTABAD, AND HOPPER VALLEY TOUR
          </p>
      ...
      <p className="text-base font-semibold px-9 py-2">
        Price: Rs 185,000 (2 Persons) • Rs 46,000 per additional person
      </p>
      ...
      <div className="mx-auto p-9">
        <h2 className="text-2xl font-bold mb-4">Air Hunza 5-Day Tour – FAQ</h2>

        {AirHunza5DaysData.map((item, index) => (
          <div
            key={index}
