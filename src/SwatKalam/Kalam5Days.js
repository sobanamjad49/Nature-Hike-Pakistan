...
import { Kalam5DaysImages, Kalam5DaysData } from "../travelData";
...
function Kalam5Days() {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "Kalam5Days", // FIXED TOUR NAME
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {Kalam5DaysImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Kalam 5 days"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            5 DAYS RELAXED KALAM, MAHODAND, MALAM JABBA & GREEN TOP
          </p>
      ...
      <div className="mx-auto p-9">
        <h2 className="text-2xl font-bold mb-4">Kalam 5-Day Tour – FAQ</h2>
        {Kalam5DaysData.map((item, index) => (
          <div
            key={index}
