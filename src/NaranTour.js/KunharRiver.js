...
import { KunharRiverImages, KunharRiverFaqData } from "../travelData";

function KunharRiver() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "KunharRiver", // FIXED TOUR NAME
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {KunharRiverImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Kunhar River"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            3 DAYS KUNHAR RIVER – NARAN TOUR
          </p>
      ...
      <p className="text-3xl font-semibold font-mono px-9 py-2">Pricing:</p>
      <span className="p-9 text-xl">
        <strong>Quad Sharing:</strong> PKR 36,000/person
      </span>
      <br />
      <span className="p-9 text-xl">
        <strong>Couple Price:</strong> PKR 82,000
      </span>
      ...
      <div className="mx-auto p-9">
        <h2 className="text-2xl font-bold mb-4">Kunhar River Tour – FAQ</h2>
        {KunharRiverFaqData.map((item, index) => (
          <div
            key={index}
      ...
      <div className="max-w-6xl mx-auto p-4 text-gray-800 space-y-6">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-white shadow rounded-lg p-4">
            ...
            <ul className="list-disc list-inside text-sm space-y-1">
              <li>Kunhar River (Naran & Kaghan belt)</li>
              <li>Saif-ul-Malook Lake (seasonal jeep)</li>
              <li>Naran Bazaar</li>
              <li>Batakundi / Burawai (time permitting)</li>
            </ul>
          </div>
        </div>
