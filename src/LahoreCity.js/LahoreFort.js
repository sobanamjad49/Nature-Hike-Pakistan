...
import { LahoreFortImages, LahoreFaqData, LahoreServicesIncluded } from "../travelData";
function LahoreFort() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "LahoreFort", // FIXED TOUR NAME
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {LahoreFortImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Lahore Fort"
      ...
      <div className="px-9 pt-6 text-gray-700">
        <p className="max-w-4xl text-base md:text-lg">
          Discover the UNESCO-listed Lahore Fort with highlights like Sheesh
          Mahal, Picture Wall, Jahangir & Shah Jahan quadrangles, and Alamgiri
          Gate. A deep-dive into Mughal, Sikh and British layers of history.
        </p>
      </div>
