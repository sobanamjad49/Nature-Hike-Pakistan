...
import { WazirKhanImages, LahoreFaqData, LahoreServicesIncluded } from "../travelData";
function WazirKhan() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "WazirKhan", // FIXED TOUR NAME
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {WazirKhanImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Wazir Khan Mosque"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">Wazir Khan Mosque</p>
      ...
      <div className="px-9 pt-6 text-gray-700">
        <p className="max-w-4xl text-base md:text-lg">
          Step into the 17th‑century Wazir Khan Mosque, famed for its kashi kari
          tilework and frescoes along the Royal Route. A masterpiece of Shah
          Jahan’s era with markets and hammam nearby.
        </p>
      </div>
