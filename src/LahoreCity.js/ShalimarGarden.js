...
import { ShalimarGardenImages, LahoreFaqData, LahoreServicesIncluded } from "../travelData";
function ShalimarGarden() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "ShalimarGarden", // FIXED TOUR NAME
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {ShalimarGardenImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Shalimar Garden"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            Shalimar Gardens – Mughal Charbagh
          </p>
          <div className="font-medium mt-3">
            Duration <br /> Half/Full Day
          </div>
        </div>
      </div>

      <div className="px-9 pt-6 text-gray-700">
        <p className="max-w-4xl text-base md:text-lg">
          Explore the UNESCO-listed Shalimar Gardens, laid out by Emperor Shah
          Jahan as a Persian-inspired charbagh with layered terraces, fountains
          and pavilions. Ideal for heritage photography and a calm walk through
          Lahore’s Mughal landscape.
        </p>
      </div>
      ...
      <div className="px-9 py-6">
        ...
        <div className="mt-4 bg-white shadow rounded-lg p-4 max-w-4xl">
          <h3 className="text-2xl font-semibold">Pricing</h3>
          <ul className="list-disc list-inside mt-2 text-sm md:text-base text-gray-800 space-y-1">
            <li>Private Customized Tour (any date)</li>
            <li>Couple / Family Packages available</li>
            <li>Group size based pricing on request</li>
          </ul>
        </div>
      </div>
      ...
      <div className="mx-auto p-9">
        <h2 className="text-2xl font-bold mb-4">Lahore Heritage Tour – FAQ</h2>
