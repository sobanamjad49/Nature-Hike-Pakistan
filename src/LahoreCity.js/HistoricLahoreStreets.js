...
import {
  HistoricLahoreStreetsImages,
  LahoreFaqData,
  LahoreServicesIncluded,
} from "../travelData";

function HistoricLahoreStreets() {
    useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "Historic Lahore Streets", // FIXED TOUR NAME
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {HistoricLahoreStreetsImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Historic Lahore Streets"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            Historic Lahore Streets – Walled City Walk
          </p>
      ...
      <div className="px-9 pt-6 text-gray-700">
        <p className="max-w-4xl text-base md:text-lg">
          Walk the royal route through Lahore’s Walled City: from iconic gates
          and bustling bazaars to frescoed mosques and hammams. Perfect for
          culture, photography, and food lovers.
        </p>
      </div>
      ...
      <div className="px-9 py-6">
        ...
        <div className="mt-4 bg-white shadow rounded-lg p-4 max-w-4xl">
          ...
          <ul className="list-disc list-inside mt-2 text-sm md:text-base text-gray-800 space-y-1">
            <li>Private Customized Tour (available any date)</li>
...
export default HistoricLahoreStreets;
