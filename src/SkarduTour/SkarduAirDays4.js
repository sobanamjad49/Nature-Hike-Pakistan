...
import { SkarduAir4DaysImages, SkarduAir4DaysData } from "../travelData";

function SkarduAirDays4() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const form = e.target;
    const formData = {
      tourName: "SkarduAirDays4", // FIXED TOUR NAME
  ...
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 m-6">
        {SkarduAir4DaysImages.map((img, index) => (
          <img
            key={index}
            src={img}
            alt="Skardu by air 4 days"
      ...
      <div className="bg-[#faf5ee] p-3">
        <div className="flex flex-col lg:flex-row justify-between m-7">
          <p className="text-3xl font-semibold lg:w-[50%]">
            4 DAYS BY-AIR TOUR TO DEOSAI, SHIGAR, AND SKARDU
          </p>
        ...
        <div className="m-7 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <p className="text-sm text-gray-600">From PKR 65,000 per person</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <p className="text-lg font-semibold">Deluxe (4-star)</p>
            <p className="text-sm text-gray-600">From PKR 85,000 per person</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <p className="text-lg font-semibold">Executive (5-star)</p>
            <p className="text-sm text-gray-600">From PKR 110,000 per person</p>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto p-9">
        <h2 className="text-2xl font-bold mb-4">Skardu By-Air 4-Day – FAQ</h2>
        {SkarduAir4DaysData.map((item, index) => (
          <div
            key={index}
